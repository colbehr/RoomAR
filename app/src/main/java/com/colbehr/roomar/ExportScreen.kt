...
fun createJsonObject(points: String, name: String): String {
    val pointsList = parseStringToPointArrays(points)
    val decodedName = URLDecoder.decode(name, "UTF-8")

    // Convert list of Offset to list of Point
    val offsetsAsPoints = pointsList.map { offset -> Point(offset.x, offset.y) }

    val yourDataClass = PointData(decodedName, points, offsetsAsPoints)
    val gson = Gson()

    return gson.toJson(yourDataClass)
}
...
fun OptionCheckbox(
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .background(
                if (isChecked) MaterialTheme.colorScheme.primary.copy(alpha = 0.2f) else Color.Transparent,
        ...
        Checkbox(
            checked = isChecked,
            enabled = isEnabled,
            onCheckedChange = { if (isEnabled) onCheckedChange.invoke() },
